...
export default function CompleteScheduleModal({ open, onClose, schedule, onSuccess }) {
  ...
  const handleConfirm = async () => {
    ...
    const completedUserIds = Object.keys(completedMap).filter(uid => completedMap[uid])
    ...
    try {
      ...
      if (res.ok) {
        toast.success(data.message || 'Xác nhận hoàn thành lịch thành công!')
        onClose()
        if (onSuccess) onSuccess()
      } else {
        toast.error(data.error || 'Có lỗi xảy ra khi xác nhận')
      }
    } catch (err) {
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth='xs'>
      ...
      <DialogContent dividers>
        <Alert severity='info' sx={{ mb: 2 }}>
          Mỗi nhân sự được tích chọn sẽ được <strong>+1 điểm rèn luyện</strong> và <strong>+1 lượt bê nước</strong> vào hồ sơ cá nhân.
        </Alert>

        <Typography variant='subtitle2' fontWeight={600} sx={{ mb: 1.5 }}>
          Danh sách người thực sự tham gia ({Object.values(completedMap).filter(Boolean).length}/{participants.length}):
        </Typography>
            ...
            return (
              <Box
                ...
                sx={{
                  ...
                  mb: 1,
                  borderRadius: 1.5,
                  bgcolor: checked ? 'rgba(40, 199, 111, 0.08)' : 'action.hover',
                  border: '1px solid',
                  borderColor: checked ? 'rgba(40, 199, 111, 0.3)' : 'transparent',
                  transition: 'all 0.2s'
                }}
              >
