...
import { assetCategoryIdFromName } from "@/libs/assetIds";
...
export async function POST(req) {
  ...
  if (!name)
    return NextResponse.json(
      { error: "Tên loại sản phẩm là bắt buộc" },
      { status: 400 },
    );
  const { categories = [] } = await getAssets();
  ...
  if (duplicate)
    return NextResponse.json(
      { error: "Loại sản phẩm đã tồn tại" },
      { status: 409 },
    );
  const category = {
    id: body.id || assetCategoryIdFromName(name),
    name,
    createdAt:
      categories.find((item) => item.id === body.id)?.createdAt ||
